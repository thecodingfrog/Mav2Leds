...
void Strip::ParseMode()
{
  ...
  {
    if (__sys_state.system_state < 2) // MAV_STATE_BOOT
    {
      __leds_mode = __led_mode.blink;
    ...
    else if (__sys_state.isArmed == 0) // disarmed: led chasing, if GPS 3D lock white color, if not 3D lock orange
    {
